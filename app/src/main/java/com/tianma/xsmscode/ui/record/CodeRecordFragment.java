...
public class CodeRecordFragment extends DaggerBackPressFragment implements CodeRecordContract.View {
    ...
    private void showSmsDetails(final RecordItem recordItem) {
        ...
        new MaterialDialog.Builder(mActivity)
                ...
                .negativeText(R.string.forward_to_wecom)
                .onNegative((dialog, which) -> forwardRecord(recordItem))
    ...
    /**
     * 手动转发该记录到企业微信（2026-09-15 新增）。
     * 复用自动转发的 Provider 通道与转发配置，同时兼作转发功能的手动测试入口。
     * 结果经 Snackbar 反馈：成功 / 未启用或未配置 / 发送失败。
     */
    private void forwardRecord(final RecordItem recordItem) {
        final SmsMsg smsMsg = recordItem.getSmsMsg();
        if (smsMsg == null) {
            return;
        }
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    android.os.Bundle r = requireContext().getContentResolver()
                            .call(uri, "forward", null, args);
                    boolean ok = r != null && r.getBoolean("ok");
                    String reason = r == null ? null : r.getString("reason");
                    if (ok) {
                        message = mActivity.getString(R.string.forward_succeed);
                    } else if ("disabled".equals(reason)) {
                        message = mActivity.getString(R.string.forward_disabled_hint);
                    } else {
                        message = mActivity.getString(R.string.forward_failed);
                    }
                } catch (Throwable t) {
                    message = mActivity.getString(R.string.forward_failed);
                }
                ...
                new android.os.Handler(android.os.Looper.getMainLooper()).post(new Runnable() {
                    ...
                    public void run() {
                        if (isAdded()) {
                            SnackbarHelper.makeLong(mRecyclerView, text).show();
                        }
                    }
                });
            }
        }, "smscodf-manual-forward").start();
    }
